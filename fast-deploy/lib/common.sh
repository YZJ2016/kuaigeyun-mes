...
_git_clean_untracked_safe() {
    ...
    git clean -fd \
        ...
        -e 'riveredge-backend/src/apps/haoligo/' \
        -e 'riveredge-backend/src/apps/haolisales' \
        -e 'riveredge-backend/src/apps/haolisales/' \
        -e 'riveredge-backend/src/apps/funide_oa' \
        -e 'riveredge-backend/src/apps/funide_oa/' \
        ...
        -e 'riveredge-frontend/src/apps/haoligo/' \
        -e 'riveredge-frontend/src/apps/haolisales' \
        -e 'riveredge-frontend/src/apps/haolisales/' \
        -e 'riveredge-frontend/src/apps/funide-oa' \
        -e 'riveredge-frontend/src/apps/funide-oa/' \
...
prompt_custom_projects_selection() {
    ...
    if [ ! -f "$custom_path/projects/registry.yaml" ]; then
        log_error "定制仓未就绪或缺少 projects/registry.yaml: ${custom_path}"
        log_error "请先完成定制仓 sync，或菜单 [4]→配置 指定 CUSTOM_REPO_PATH"
        return 1
    fi

    local -a ids=() labels=()
    while IFS=$'\t' read -r pid desc; do
        [ -n "$pid" ] || continue
        ids+=("$pid")
        labels+=("$desc")
    done < <("$py" "$gen" --list-custom-projects "$custom_path")

    n="${#ids[@]}"
    if [ "$n" -eq 0 ]; then
        log_error "registry 中无可选定制项目: ${custom_path}/projects/registry.yaml"
        return 1
    fi
