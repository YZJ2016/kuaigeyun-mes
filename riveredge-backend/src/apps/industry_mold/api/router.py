...
from apps.industry_mold.models.material_arrival import MoldMaterialArrival
from apps.industry_mold.models.program_sheet import MoldProgramSheet
...
@router.get(
    "/health",
    dependencies=[Depends(require_permission_codes("industry-mold:entry:read"))],
)
async def health(tenant_id: int = Depends(get_current_tenant)) -> dict:
    return {"ok": True, "app": "industry-mold", "tenant_id": tenant_id}


@router.get(
    "/program-sheets",
    response_model=List[MoldProgramSheetResponse],
    dependencies=[Depends(require_permission_codes("industry-mold:program:read"))],
)
...
@router.post(
    ...
    dependencies=[Depends(require_permission_codes("industry-mold:program:create"))],
)
...
@router.get(
    ...
    dependencies=[Depends(require_permission_codes("industry-mold:arrival:read"))],
)
...
@router.post(
    ...
    dependencies=[Depends(require_permission_codes("industry-mold:arrival:create"))],
)
...
@router.post(
    ...
    response_model=MoldMaterialArrivalResponse,
    dependencies=[Depends(require_permission_codes("industry-mold:arrival:update"))],
)
