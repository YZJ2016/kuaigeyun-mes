...
class AnnouncementService:
    @staticmethod
    def _is_active_published(row: KuaioaAnnouncement, *, now: datetime) -> bool:
        """按 ORM datetime 判断已发布公告是否在生效窗口内。"""
        ...
        *,
        ...
        for row in rows:
            if active_only or published_only or status == "published":
                if not self._is_active_published(row, now=now):
                    continue
    ...
    async def create_announcement(
        ...
        payload: dict[str, Any] = {
            ...
            "effective_at": parse_business_datetime(data.effective_at),
            "expires_at": parse_business_datetime(data.expires_at),
            "status": "draft",
        }
    ...
    async def update_announcement(
        ...
        for key in ("effective_at", "expires_at"):
            if key in payload:
                payload[key] = parse_business_datetime(payload[key])
