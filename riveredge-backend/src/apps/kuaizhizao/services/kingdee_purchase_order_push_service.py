"""业务适配：采购订单 → 金蝶采购订单（PUR_PurchaseOrder）。"""
...
from core.services.integration.kingdee_galaxy_api_presets import (
    PUR_PURCHASE_ORDER_DEFAULT_BILL_TYPE,
    PUR_PURCHASE_ORDER_FORM_ID,
)
from core.utils.timezone_utils import to_api_isoformat
from infra.exceptions.exceptions import BusinessLogicError
from infra.services.business_config_service import BusinessConfigService

DEFAULT_FORM_ID = PUR_PURCHASE_ORDER_FORM_ID
TARGET_TYPE = "kingdee_purchase_order"
TARGET_PROFILE = "kingdee_pur_purchaseorder"
SOURCE_TYPE = "purchase_order"
PUSHABLE_STATUSES = frozenset({"AUDITED", "CONFIRMED", "APPROVED", "RELEASED", "IN_PROGRESS", "COMPLETED"})
PUSH_DISABLED_MESSAGE = "金蝶采购订单推送未启用"


def purchase_order_push_skip_reason(
    order: PurchaseOrder,
    *,
    ...
    if review != "APPROVED" and status not in PUSHABLE_STATUSES:
        return f"采购订单状态 {status or '-'} / 审核 {review or '-'} 不可推送"
...
async def _resolve_supplier_number(tenant_id: int, order: PurchaseOrder, cfg: Dict[str, Any]) -> Optional[str]:
    configured = str(cfg.get("supplier_number") or "").strip()
    if configured:
        return configured
    try:
        ...
        supplier = await Supplier.get_or_none(
            tenant_id=tenant_id,
            id=int(order.supplier_id or 0),
            deleted_at__isnull=True,
        )
        if supplier and str(supplier.code or "").strip():
            return str(supplier.code).strip()
    except Exception as exc:
        logger.warning("resolve supplier number failed order_id={} err={}", order.id, exc)
    return None


def build_kingdee_purchase_order_model(
    *,
    order: PurchaseOrder,
    ...
    for item in items:
        unit_raw = str(item.unit or "").strip()
        unit_number = (
        ...
        entry = {
            ...
            "FQty": _to_float(item.ordered_quantity),
            "FPrice": _to_float(item.unit_price),
            "FTaxPrice": _to_float(item.unit_price),
            "FEntryTaxRate": _to_float(getattr(order, "tax_rate", None) or 0),
            "FDeliveryDate": _date_string(item.required_date or order.delivery_date),
        }
        if item.notes:
    ...
    if entries:
        model["FPOOrderEntry"] = entries

    default_field_map = {
        "bill_no": "FBillNo",
        "bill_date": "FDate",
        "bill_type_number": "FBillTypeID.FNUMBER",
        "purchase_org_number": "FPurchaseOrgId.FNumber",
        "supplier_number": "FSupplierId.FNumber",
        "remarks": "FNote",
    }
    ...
    def local_value(key: str) -> Any:
        ...
        if key == "bill_type_number":
            return config.get("bill_type_number") or PUR_PURCHASE_ORDER_DEFAULT_BILL_TYPE
        if key == "purchase_org_number":
            return config.get("purchase_org_number") or config.get("org_number")
        if key == "supplier_number":
            return supplier_number or config.get("supplier_number")
        if key == "remarks":
            return _compact_text("星技谷采购订单", order.order_code, order.notes)
...
class KingdeePurchaseOrderPushService:
    ...
    async def push_purchase_order(
        ...
        *,
        ...
        skip_reason = purchase_order_push_skip_reason(
            order,
        ...
        try:
            ...
            logger.warning(
                "采购订单推送金蝶失败 tenant_id={} purchase_order_id={} err={}",
                tenant_id,
                purchase_order_id,
                exc,
            )
            if bool(config.get("fail_on_error", False)):
                raise BusinessLogicError(f"推送金蝶采购订单失败：{exc}")
    ...
    async def get_push_config(self, tenant_id: int) -> Dict[str, Any]:
        ...
        purchase = (biz_config.get("parameters", {}) or {}).get("purchase", {}) or {}
        raw = purchase.get("kingdee_purchase_order_push") or {}
    ...
    async def _push_now(
        ...
        *,
        ...
        supplier_number = await _resolve_supplier_number(tenant_id, order, config)
        if not supplier_number and not dry_run:
            raise BusinessLogicError("缺少供应商编码（Supplier.FNumber），无法推送金蝶采购订单")

        first_code = items[0].material_code if items else None
        first_unit = str(getattr(items[0], "unit", None) or "").strip() if items else ""
        ...
        async def _persist(result: Dict[str, Any]) -> None:
            await DocumentRelation.create(
                ...
                target_name=str(result.get("target_name") or "金蝶采购订单"),
                relation_type="source",
                relation_mode="push",
                relation_desc=str(result.get("relation_desc") or "采购订单推送金蝶采购订单"),
