...
class PackageBase(BaseModel):
    ...
    max_branch_organizations: Optional[int] = Field(default=None, ge=0, description="最大分支组织数量（null=不限制）")
    is_active: bool = Field(default=True, description="是否激活")
    allow_pro_apps: bool = Field(default=False, description="是否允许使用 PRO 应用")
    allowed_app_codes: List[str] = Field(default_factory=list, description="允许加载的应用编码白名单（空=不限制）")
...
class PackageUpdate(BaseModel):
    ...
    allowed_app_codes: Optional[List[str]] = Field(None, description="允许加载的应用编码白名单（空=不限制）")
