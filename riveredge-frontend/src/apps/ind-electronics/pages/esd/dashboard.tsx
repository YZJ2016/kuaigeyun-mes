...
/** ESD 专项看板：总览 + 主数据厂区切换。 */
export default function IndElectronicsEsdDashboardPage() {
  ...
  const { data: plantsResult } = useRequest(() => indElectronicsEsdApi.getBoardPlants(), {
    ready: !perms.enabled || perms.canRead,
  });
  const { data: board, loading } = useRequest(
    () => indElectronicsEsdApi.getBoard(plantId, visitMode),
    {
      ready: !perms.enabled || perms.canRead,
  ...
  const plantOptions = useMemo(
    () => [
      { label: t('app.ind-electronics.esd.plantOverview'), value: 'all' },
  ...
  const kpis: ModuleKpiDef[] = useMemo(
    () => [
      {
        key: 'due',
        title: t('app.ind-electronics.esd.kpi.due'),
        value: m?.spot_check_due ?? 0,
        icon: <DashboardOutlined style={{ fontSize: 24, color: '#fff' }} />,
        gradient: 'linear-gradient(135deg, #1890ff 0%, #36cfc9 100%)',
        sideMetrics: [
          { label: t('app.ind-electronics.esd.kpi.equipment'), value: m?.total_count ?? 0 },
        ],
      },
      {
        key: 'rate',
        title: t('app.ind-electronics.esd.kpi.rate'),
        value: `${m?.spot_check_rate ?? 0}%`,
        progress: m?.spot_check_rate ?? 0,
        icon: <CheckCircleOutlined style={{ fontSize: 24, color: '#fff' }} />,
        gradient: 'linear-gradient(135deg, #52c41a 0%, #95de64 100%)',
        sideMetrics: [
          { label: t('app.ind-electronics.esd.kpi.done'), value: m?.spot_check_done ?? 0 },
          { label: t('app.ind-electronics.esd.kpi.pending'), value: m?.spot_check_pending ?? 0 },
        ],
      },
      {
        key: 'alerts',
        title: t('app.ind-electronics.esd.kpi.alerts'),
        value: m?.alert_count ?? 0,
        icon: <AlertOutlined style={{ fontSize: 24, color: '#fff' }} />,
        gradient: 'linear-gradient(135deg, #ff4d4f 0%, #ff7875 100%)',
        onClick: () => navigate('/apps/ind-electronics/esd/inspection'),
        sideMetrics: [
          {
            label: t('app.ind-electronics.esd.kpi.reviewPending'),
  ...
  const alertFeedItems: ModuleFeedItem[] = useMemo(
    () =>
      alerts.slice(0, 12).map((row, idx) => ({
        ...
        subtitle: row.detail,
        tag: {
          label: t(`app.ind-electronics.esd.alertKind.${row.kind}`),
          ...
          <Space orientation="vertical" size={12} style={{ width: '100%' }}>
            ...
            >
              <Space size={8} wrap>
                ...
                <Text strong>{t('app.ind-electronics.menu.esdDashboard')}</Text>
                {board?.spot_check_date ? (
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    {t('app.ind-electronics.esd.spotCheckDate', { date: board.spot_check_date })}
                  </Text>
                ) : null}
                <Text type="secondary" style={{ fontSize: 12 }}>
        ...
        actionRow={
          <ModuleActionMasonry>
            ...
            {showMasonryCard(loading && !board, statusStructureRows.length > 0, masonryEmptyFallback) ? (
              <ModuleActionPanel
                layout="masonry"
                title={t('app.ind-electronics.esd.statusStructureTitle')}
                loading={loading && !board}
              ...
              >
                <Table
                  ...
                  columns={[
                    {
                      title: t('app.ind-electronics.esd.colStatus'),
                      dataIndex: 'label',
                      render: (label: string) => <MarkerTag color="processing">{label}</MarkerTag>,
                    },
                    {
                      title: t('app.ind-electronics.esd.colCount'),
                      dataIndex: 'count',
                      width: 72,
                    },
                  ]}
