 ...
 */
export default function IndElectronicsLabelOemPage() {
  const { t } = useTranslation();
  const { message } = App.useApp();
  const perms = useResourcePermissions('ind-electronics:label-oem');
  ...
  const packTab = useMemo(
    () => ({
      key: 'oem-packs',
      label: t('app.ind-electronics.labelOem.tabPacks'),
      children: (
        <Space orientation="vertical" style={{ width: '100%' }} size="medium">
          <Alert
            type="info"
            showIcon
            title={t('app.ind-electronics.labelOem.packHintTitle')}
            description={t('app.ind-electronics.labelOem.packHintBody')}
          />
          {placeholderUuid ? (
            <Alert
              type="warning"
              showIcon
              title={t('app.ind-electronics.labelOem.placeholderTitle')}
              description={t('app.ind-electronics.labelOem.placeholderBody', {
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setPlaceholderUuid(res.placeholder_template_uuid || '');
                message.success(
                  t('app.ind-electronics.labelOem.ensureSuccess', {
          ...
          >
            {t('app.ind-electronics.labelOem.ensureAll')}
          </Button>
          <Table
            size="small"
            rowKey="pack_code"
            ...
            columns={[
              {
                title: t('app.ind-electronics.labelOem.packCode'),
                dataIndex: 'pack_code',
              },
              {
                title: t('app.ind-electronics.labelOem.packName'),
                dataIndex: 'pack_name',
              },
              {
                title: t('app.ind-electronics.labelOem.barcodeKind'),
              ...
              {
                title: t('app.ind-electronics.labelOem.applied'),
                dataIndex: 'applied',
                width: 90,
                render: (v: boolean) => (
                  <Tag variant="filled">
                    {v
                      ? t('app.ind-electronics.labelOem.appliedYes')
                      : t('app.ind-electronics.labelOem.appliedNo')}
                  </Tag>
                ),
              },
              {
                title: t('app.ind-electronics.labelOem.templateBound'),
                dataIndex: 'template_bound',
                width: 110,
                render: (v: boolean) => (
                  <Tag variant="filled" color={v ? 'success' : 'warning'}>
                    {v
                      ? t('app.ind-electronics.labelOem.templateBoundYes')
                      : t('app.ind-electronics.labelOem.templateBoundNo')}
                  </Tag>
                ),
              },
            ]}
