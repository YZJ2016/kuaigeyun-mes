...
export const indElectronicsEsdApi = {
  getCatalog: () => apiRequest<EsdCatalog>('/apps/ind-electronics/esd/catalog', { method: 'GET' }),
  updateCatalog: (project_types: Partial<EsdProjectType> & { code: string }[]) =>
    apiRequest('/apps/ind-electronics/esd/catalog', {
      method: 'PUT',
      data: { project_types },
    }),
  ensureCatalog: () =>
    apiRequest('/apps/ind-electronics/esd/catalog/ensure', { method: 'POST' }),
  getBoardPlants: () =>
    apiRequest<{ items: { id: number; uuid: string; code: string; name: string }[]; total: number }>(
      '/apps/ind-electronics/esd/board/plants',
      { method: 'GET' },
    ),
  getBoard: (plantId?: number, visitMode = false) =>
    apiRequest<EsdBoard>('/apps/ind-electronics/esd/board', {
      method: 'GET',
